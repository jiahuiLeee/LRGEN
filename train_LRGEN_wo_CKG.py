from utils import *
from models.LRGEN import LRGEN_wo_CKG
...
def evaluate(model, test_graph, test_dataloader, test_fin_index_emb, test_labels, classify_criterion):
    ...
    all_probs = []
    with torch.no_grad():
            subgraph_triple = None
            out = model(test_graph, subgraph_triple, test_fin_index_emb, last_batch, emb_list)
            ...
            return accuracy, classify_loss, f1, precision, recall, auc
...
model = LRGEN_wo_CKG(
    ...
    hidden_dim=config['LRGEN']['hidden_dim'],
    num_types=config['LRGEN']['num_node_type'],
    num_relations=config['LRGEN']['num_relations'],
    n_heads=config['LRGEN']['num_head'],
...
log_dir = f'./logs/LRGEN_woCKG_HGT_{timestamp}'
...
with open(log_file, mode='w', newline='') as file:
    ...
    for epoch in range(num_epochs):
        ...
        subgraph_triple = None
        out = model(train_graph, subgraph_triple, train_fin_index_emb, last_batch, emb_list)
        ...
        # 测试
        test_accuracy, test_classify_loss, f1, precision, recall, auc = \
        ...
        print(f"Train Loss1: /, Train Loss2: {classify_loss:.4f}, Train Accuracy: {train_accuracy}")
        print("*****test*****")
        print(f"Test Loss1: /, Test Loss2: {test_classify_loss:.4f}, Test Accuracy: {test_accuracy}")
        print(f"Test F1: {f1}")
        ...
        # 保存训练数据到CSV
        writer.writerow([
            epoch + 1, train_accuracy, 'None', classify_loss.item(),
