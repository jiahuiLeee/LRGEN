...
class ourGraphClassificationDataset(GraphDataset):
    def __init__(
        self,
        graph_file_dir='/home/ljh/RGEN_A/data/CKG/json_output/2023',
        rw_hops=64,
        subgraph_size=64,
        restart_prob=0.8,
        positional_embedding_size=32,
    ...
    def _create_dgl_graph(self, path):
        ...
        for node in nodes:
            # print(node)
            node_id = node['id']
            if 'full_name' in node['properties']:
                node_name = node['properties']['full_name']
            else:
                node_name = node['properties']['bz_item']
